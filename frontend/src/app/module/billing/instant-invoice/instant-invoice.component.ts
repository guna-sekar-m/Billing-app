...
@Component({
  ...
  styleUrls: ['./instant-invoice.component.css']
})
export class InstantInvoiceComponent {
  suggestions : any;
  invoicedate : string;
  invoicedetails : any = signal({});
  invoiceItems : any = signal([{Invoice_ID:'', Item_Name: '',Discription:'', Quantity: 1,Sale_Price: '',Tax_Status :'Exclusive',Tax_Rate:0, Amount:0, Discount: 0 }]);
  ...
  async getnewinvoice(){
    ...
    let today = new Date();
    this.invoicedetails.mutate((data : any)=>data.Invoice_Date= today.getMonth() + '/' + today.getDate() + '/' + today.getFullYear());
  }
  ...
  addItem(){
    this.invoiceItems.mutate((data : any) => data.push({Invoice_ID:'',Item_Name: '',Discription:'', Quantity: 1,Tax_Status :'Exclusive',Tax_Rate:0,Sale_Price: '',Amount:0,Discount: 0,}));
  }
